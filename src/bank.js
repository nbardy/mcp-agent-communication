...
// ---------- helpers ----------
const now = () => Date.now();
const matches = ({ agent_ids = [], tags = [] }, m) => (agent_ids.length === 0 || agent_ids.includes(m.agent_id)) &&
    (tags.length === 0 || m.tags.some(t => tags.includes(t)));
export const handlePut = (r) => {
    const msg = {
        id: randomUUID(),
        ts: now(),
...
export const handleList = (r) => ({
    messages: messages.filter(m => matches(r, m)),
});
export const handleRead = (r) => ({
    messages: messages.filter(m => m.ts > (r.since ?? 0) && matches(r, m)),
});
export const handleGather = async (r) => {
    const required = new Set(r.agent_ids.flatMap(a => r.tags.map(t => `${a}|${t}`)));
    const seen = new Set();
    const checkMsg = (m) => {
        if (matches(r, m)) {
...
// -------------- dispatch ---------------
export const dispatch = async (req) => {
